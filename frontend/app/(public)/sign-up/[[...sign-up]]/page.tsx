  ...
  return (
    <div className="min-h-screen bg-white text-black">
      <main className="mx-auto max-w-md px-4 py-16">
        ...
        {/* Error (live region for accessibility) */}
        {errorMessage ? (
          <div
            ...
            aria-live="assertive"
          >
            <p className="text-sm font-medium text-red-900">Sign-up error</p>
        ...
        <div className="mt-8 rounded-2xl border border-zinc-200 p-6">
          <button
            ...
            disabled={isLoading}
            aria-busy={isLoading}
            className="group relative w-full inline-flex items-center justify-center gap-2 rounded-lg border border-black bg-black px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-black disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isLoading ? (
          ...
          {/* Benefits (monochrome, minimal) */}
          <ul className="mt-6 space-y-3">
            <li className="flex gap-3">
              <span className="mt-2 h-1.5 w-1.5 rounded-full bg-zinc-800" />
              <p className="text-sm text-zinc-700">
                Create and manage organizations for your open-source projects.
              </p>
            </li>
            <li className="flex gap-3">
              <span className="mt-2 h-1.5 w-1.5 rounded-full bg-zinc-800" />
              <p className="text-sm text-zinc-700">
                Connect with contributors and build meaningful software together.
              </p>
            </li>
            <li className="flex gap-3">
              <span className="mt-2 h-1.5 w-1.5 rounded-full bg-zinc-800" />
              <p className="text-sm text-zinc-700">
                Track progress and coordinate work without extra overhead.
              </p>
            </li>
          </ul>
        </div>
